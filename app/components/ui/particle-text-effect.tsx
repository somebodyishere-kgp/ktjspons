...
const DEFAULT_WORDS = ["KSHITIJ", "2026", "INNOVATION", "TECHNOLOGY", "CREATIVITY", "EXCELLENCE"]
...
export function ParticleTextEffect({ 
  ...
  const canvasSizeRef = useRef({ width: 1000, height: 500 })
  ...
  const resizeCanvas = () => {
    ...
    if (!canvas || !container || !autoResize) return
    ...
    // Redraw current word with new dimensions
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Generate new color with red theme for KSHITIJ
    const newColor = {
      r: Math.random() * 100 + 200, // More red (200-255)
      g: Math.random() * 50, // Less green (0-50)
      b: Math.random() * 50, // Less blue (0-50)
    }
  ...
  const animate = () => {
    ...
    if (frameCountRef.current % onChangeInterval === 0) {
  ...
  }, [words, fontSize, fontFamily, autoResize, backgroundColor, motionBlurOpacity, onChangeInterval])
